...
class PayloadParser:
    def sendVariables():
        return PayloadParser.globalMoney
    def parse_payload(self, payload, gamestate_manager):
        ...
        global globalMoney
        global globalRound
        globalArmour = state_info['armor']
        globalKills = match_stats['kills']
        globalMoney = state_info['money']
        globalRound = map_info['round']
        change_flags = {}
        ...
        if 'round' in payload and 'phase' in payload['round']:
            ...
            if payload['round']['phase'] != gamestate_manager.gamestate.round_phase:
                ...
                roundchanged = True
            if roundchanged and payload['round']['phase'] == 'freezetime':
                with open("data.txt", "w") as dataFile:
                    dataFile.write(str(globalArmour) + "\n" + str(globalKills) + "\n" +  str(globalMoney) + "\n" +  str(globalRound))
                dataFile.close()
                print(globalArmour, globalKills, globalMoney, globalRound)
